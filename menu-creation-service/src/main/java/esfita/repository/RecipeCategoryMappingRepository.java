...
public interface RecipeCategoryMappingRepository extends JpaRepository<RecipeCategoryMappingHib, Integer> {
	...
	@Query("FROM RecipeCategoryMappingHib WHERE recipeFk IN :ids AND status = 'A'")
	List<RecipeCategoryMappingHib> findByRecipeFkIn(@Param("ids") Set<Integer> ids);

	List<RecipeCategoryMappingHib> findByRecipeFkIn(Iterable<Integer> ids);
