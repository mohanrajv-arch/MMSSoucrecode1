...
public interface RecipeMealMappingRepository extends JpaRepository<RecipeMealMappingHib, Integer> {
	...
	RecipeMealMappingHib findByMealAndRecipeActive(@Param("mealFk") int mealFk, @Param("recipeFk") int recipeFk);

	@Query("SELECT r FROM RecipeMealMappingHib r WHERE r.id = :id AND r.status = 'A'")
	RecipeMealMappingHib findById(@Param("id") int id);

	List<RecipeMealMappingHib> findByRecipeFkIn(Iterable<Integer> ids);
