...
public interface MealSetTemplateHRepository extends JpaRepository<MealSetTemplateHHib, Integer> {
	...
	@Query(value = "SELECT " + "m.meal_type_name, " + "COUNT(m.id) AS total_template_count, "
			+ "COUNT(*) FILTER (WHERE m.status = 'A') AS active_template_count, "
			+ "COUNT(DISTINCT m.meal_type_name) AS meal_type_count " + "FROM dbo.meal_set_template_h m "
			+ "GROUP BY m.meal_type_name " + "ORDER BY m.meal_type_name", nativeQuery = true)
	List<Object[]> getTemplateSummaryByMealType();

	@Query("SELECT m FROM MealSetTemplateHHib m "
			+ "WHERE m.mealTypeFk = :mealTypeFk AND m.status = 'A' AND m.approvalStatus = 0 "
			+ "ORDER BY m.templateName DESC")
