...
public class TracingConfig {
	...
	@Bean
	 Tracer micrometerTracer() {
		...
		Attributes serviceAttrs = Attributes.of(AttributeKey.stringKey("service.name"), "meal-set-template-services");
		...
		return new OtelTracer(openTelemetry.getTracer("meal-set-template-services"), new OtelCurrentTraceContext(),
				span -> {
				});
	}
}
