...
const ViewMealSetMenu = () => {
  ...
  const enhancedData = useMemo(() => {
    ...
    // Calculate totals from actual API data (assuming categoryList is in response)
    ...
    const totalPortion = data.categoryList?.reduce((sum, cat) => {
      const categoryPortion = cat.recipes?.reduce((catSum, recipe) => catSum + (recipe.portionSize || 0), 0) || 0;
    ...
    return {
      ...
      categoryList: data.categoryList?.map(cat => {
        ...
        const categoryPortion = cat.recipes?.reduce((sum, recipe) => sum + (recipe.portionSize || 0), 0) || 0;
        
        return {
          ...cat,
          totalCost: categoryCost,
          totalPortion: categoryPortion,
          recipes: cat.recipes || [] // Use actual recipes from API
        };
      }) || [],
      totalCost: totalCost,
      totalPortion: totalPortion,
      // Use totalCategories and totalRecipes from API response
      totalCategories: data.totalCategories || 0,
      totalRecipes: data.totalRecipes || 0,
      approver: data.approver || 'N/A',
      userName: data.userName || data.approver || 'N/A'
    };
  }, [data]);
  ...
  const recipeChartData = useMemo(() => {
    ...
    return category.recipes.map((recipe, index) => ({
      ...
      y: recipe.recipeCost || 0, // Using recipeCost from API
  ...
  // Main chart options with improved click functionality and centered label
const chartOptions = useMemo(() => {
  ...
  // If no data for recipe level, show message
        ...
        render: function () {
          ...
          if (currentData.length > 0) {
            ...
            this.customLabel = this.renderer.text(labelText, centerX, centerY, true)
              .attr({
                align: "center",
                zIndex: 0  // Set low z-index for the label
              })
              .css({ 
                fontSize: "14px", 
                color: "#1f2937", 
            ...
            this.customLabel.attr({
              x: centerX - (bbox.width / 18),
              y: centerY - (bbox.height / 6)
            });
          }
        }
      }
  ...
  style: {
    ...
    plotOptions: {
      pie: {
        innerSize: "70%", // Slightly larger inner for better center space
...
}, [enhancedData, categoryChartData, recipeChartData, chartLevel, selectedCategory, totalCostAllCategories, selectedCategoryTotal]);
  ...
  useEffect(() => {
    ...
    const apiUrl = `https://kelvinmms.com:8443/api-gateway-mms/menu-creation-service/mealSetMenuController/mealSetMenuViewById/${id}`;
  ...
  const { templateName, MealType, approvalStatusStr, userName, approver, totalCategories, totalRecipes } = enhancedData;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-2.5 flex items-center justify-between">
        <h1 className="text-sm font-semibold text-gray-900">View Meal Set Menu</h1>
        <button
            ...
            <div className="bg-white rounded-lg border border-gray-200 h-full flex flex-col shadow-sm">
                        ...
                        <tbody>
                          {filteredItems.map((item, index) => (
                            ...
                            >
                              ...
                              <td className="p-1 text-center text-xs font-medium text-gray-900">{formatQuantity(item.portion || 0, projectSettings?.quantityDecimalPlaces || 2)}</td>
              ...
              <div className="p-2 border-t border-gray-200/50 bg-gradient-to-r from-gray-50/50 to-blue-50/50">
                <div className="flex justify-between text-xs font-semibold text-gray-900">
                  ...
                  <span>{formatAmount(tableTotalCost || 0, projectSettings?.costDecimalPlaces || 2)}/{formatQuantity(tableTotalPortion || 0, projectSettings?.quantityDecimalPlaces || 2)}</span>
                </div>
              </div>
            </div>
          </div>
...
export default ViewMealSetMenu;